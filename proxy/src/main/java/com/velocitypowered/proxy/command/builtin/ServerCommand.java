...
public class ServerCommand implements SimpleCommand {
  ...
  private void outputServerInformation(Player executor) {
    ...
    executor.sendMessage(Identity.nil(), Component.translatable(
        "velocity.cmd.server-current-server",
    ...
    if (servers.size() > MAX_SERVERS_TO_LIST) {
      executor.sendMessage(Identity.nil(), Component.translatable(
          "velocity.cmd.server-too-many", NamedTextColor.RED));
      return;
    }

    // Assemble the list of servers as components
    TextComponent.Builder serverListBuilder = Component.text()
        .append(Component.translatable("velocity.cmd.server-available",
            NamedTextColor.YELLOW))
  ...
  private TextComponent formatServerComponent(String currentPlayerServer, RegisteredServer server) {
    ...
    if (connectedPlayers == 1) {
      playersTextComponent = Component.translatable("velocity.cmd.server-tooltip-player-online");
    } else {
      playersTextComponent = Component.translatable("velocity.cmd.server-tooltip-players-online");
    }
    ...
    if (serverInfo.getName().equals(currentPlayerServer)) {
      serverTextComponent = serverTextComponent.color(NamedTextColor.GREEN)
          .hoverEvent(
              showText(
                  Component.translatable("velocity.cmd.server-tooltip-current-server")
      ...
      serverTextComponent = serverTextComponent.color(NamedTextColor.GRAY)
          ...
          .hoverEvent(
              showText(Component.translatable("velocity.cmd.server-tooltip-offer-connect-server")
                  .append(Component.newline())
                  .append(playersTextComponent))
          );
    }
  ...
  public boolean hasPermission(final SimpleCommand.Invocation invocation) {
    return invocation.source().getPermissionValue("velocity.cmd.server") != Tristate.FALSE;
  }
}
